...
|---|---|---|---|
| Q-R-1 | Where is the promotion act, and does Themis (the system) hold it? | promotion = the existing two doors, unchanged: catalog registration (executability) and deployment-anchor reliance (`skills[]`); both are Governance acts already; Themis does NOT become the registry of runtime methods (D-C-2 forbids the dependency); Themis learns the method only through the commission | OPEN |
