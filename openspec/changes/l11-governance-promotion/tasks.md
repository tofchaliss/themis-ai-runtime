...
tree and its loaders; the pin moves land with `rsys@6`. LANDED in I-M1
(2026-09-26) except the status/preflight rendering of `decision_ref`.

- [x] `policies/decisions/` schema (`id, kind, target{name@version, hash},
      evidence[]{criterion, package_object_id, record_root}, rationale,
      decided_at, actor{commit:<author> | key:<KeyID>}`), closed, strictjson
- [x] Decision-record loader (read-only): existence, hash, `target` two-way
      against the referencing entry
- [x] Skill catalog `version: 2`: `decision_ref` required on every entry;
      loader refuses missing/mismatched; decision records written for
      the four existing entries from the archived ratifications
- [x] Anchors registry `version: 2`: same for every anchor entry
- [x] Wall tests: zero `os` writers in `skills`, `deployment`, decision
