...
- **Governed-inclusion disposition (owner-refined 2026-09-06):** every governed slot has a deterministic effective disposition among `required/undroppable` · `optional/undroppable` · `optional/droppable` · `excluded_by_contract`. Explicit `droppable` is required for capacity-driven omission; absence of the declaration resolves to undroppable (fail-closed default — omission only ever makes dropping less aggressive). A workflow contract may not rely on undeclared behavior to exclude governed evidence. (No universal at-least-one-undroppable floor — some analytical workflows legitimately run without governed records.)
