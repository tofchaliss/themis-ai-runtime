...
| Scenario-3 regression: authority-disposition args die at schema | `TestAuthorizeDecisionTable/scenario-3` (`requires_human_decision` ⇒ invalid-args unknown-field, whole call); `TestRegistryFailsClosed/authority-disposition param` + three F5 stem cases (approved, trust_class, human_approved) — all branch-asserted after the close-out review caught the misfiring suite |
...
| Operational proof (Q-L4-9) | Deterministic half ✓: `TestProviderRoundTrip` (scripted provider emits structured tool_calls through the real Model Interface seam → Authorize → execute → tool message → completion) + `TestHandlePipeline`. **Live half DONE 2026-09-06**: `TestLiveToolProof` vs qwen2.5:7b (owner-authorized after the recorded qwen2.5-coder protocol failure) — all six cases through the real seam: authorized+sentinel completion, live class-only not-available, target-refused, typed error, invalid-args, classified evidence. PASS 13.66s |
