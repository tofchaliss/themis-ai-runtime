...
| Operational proof (Q-L4-9) | Deterministic half ✓: `TestProviderRoundTrip` (scripted provider emits structured tool_calls through the real Model Interface seam → Authorize → execute → tool message → completion) + `TestHandlePipeline`. **Live half PENDING**: qwen2.5-coder:7b failed the tool-protocol smoke (emits tool JSON as content, structured tool_calls null — recorded evidence 2026-09-06); a tools-capable model pull awaits owner authorization; content-as-action parsing rejected on principle |
