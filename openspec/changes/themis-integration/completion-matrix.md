...
| 2 | Finding read | `GET /findings/{id}` (FindingView) 🟢 | L4 `ThemisSeam.Read` → `governed-record`; stand-in JSON seam 🟢 (T-M2) | HTTP seam with projection, `themis_contract` pin (D-I-3) | 🟡 | I-M1 |
| 3 | Finding scope | UUID identity 🟢 | `themis_scope` prefix match 🟢; `uuid` syntax + subject-bound `@input:finding` decided (D-I-4, D-I-9) | immutable task-bound scope; L7 binds, L4 enforces, L6 records | 🟡 | I-M1 (`uuid`); D-I-9 hardening |
| 4 | Workflow commissioning | **no record that work was commissioned for a Finding** | L9 instantiation from human inputs 🟢 | the Finding → task link exists only in the envelope inputs and later in the proposal evidence; Themis holds no commissioning act | 🔴 | grill: does Themis record a commissioning act (who, which Finding, which skill@version, which anchor), or is the proposal's evidence the only link? Not decided by D-I-4 |
| 5 | Execution | — | L5 sealed workspace, L6 record plane 🟢 (rsys@3..5 live); L5 witnesses decided (D-W-2/3/6) | evidence the record plane can replay | 🟡 | W-M1, W-M2 |
...
| 7 | Evidence reconstruction | Governance adapter `harness` (decided, D-I-7) | `state`, `deployment`, `verification`, `verification/seam` read-only surface 🟢; `seam.ReconstructEvent` exported 🟢 | `intake.Resolve` D-T-1..6 🟢 in the stand-in (T-M3); five-link replay + constitution-keyed obligation decided (D-W-5) | 🟡 | I-M3 (move + five-link + witnessing table) |
...
| 11 | Subagents (L8) | — | **implemented and verified**: D-L8-1..21 / C-L8-1..21 locked, archived 2026-09-23, live COMPLETED under rsys@5 (Addendum F) 🟢 | **integration surface undecided:** does a delegate inherit the parent's Themis scope (row 3)? may a delegate read Themis at all? how do `l8-delegation` events appear in the intake evidence view (D-T-7 "model turns" today lists `model-turn` only)? | 🔴 | grill: L8 × Themis scope, L8 × evidence view. NOT a re-grill of L8 itself. |
| 12 | Ratchet (L11) ↔ Governance | Governance has no notion of skill/workflow promotion | L11 criteria and regression sets 🟢 (`walk-report-score-delta@1`) | promotion boundary: who decides a skill revision is promotable, on which L11 evidence, and whether that is a Themis Governance act | 🔴 | grill: promotion as a Governance act vs. harness-internal ratchet; D-L9-16 "two gates" is the starting point |
...
| 14 | Authentication | API keys, scopes, `authadmin` 🟢 | scoped capabilities per task 🟢 | integration identity: read key (harness), product write key (operator), write key (decider); key ids in evidence, never values (D-I-8) | 🟡 | I-M1 (seam key handling), I-M5 (host) |
| 15 | Harness module identity | — | module path collides with the Themis repo URL | rename + pinned pseudo-version (D-I-2) | 🟡 | I-M0 |
