...
|---|---|---|---|---|---|---|
| 1 | Security truth | Governance owns Findings, Proposals, Positions 🟢 (Phase-3, VM-verified 2026-09-22) | no write path exists; four-class authority vocabulary 🟢; stand-in dissolved 🟢 (I-M4) | authority boundary: harness never initiates a Governance act (D-I-1); no Position mechanism outside Themis (D-I-5) | 🟡 | Themis commit (row 13) |
...
| 7 | Evidence reconstruction | `internal/governance/adapters/harness` implemented 🟢 (I-M3, uncommitted in the Themis repo): five-link replay, witnessing table, evidence mapping; tests over the real fixtures and forged records | `state`, `deployment`, `verification`, `verification/seam` read-only surface 🟢; `seam.ReconstructEvent` exported 🟢 | `intake.Resolve` D-T-1..6 🟢 in the stand-in (T-M3); five-link replay + constitution-keyed obligation decided (D-W-5); provenance-bearing real-walk fixture 🟢 (I-M2) | 🟡 | Themis commit + go.mod pin after the harness push; `make check` there |
| 8 | Proposal | `raiseProposal` with `harness-execution/v1` evidence, derived trust validated, correspondence + Business Verification 🟢 (I-M3, uncommitted in the Themis repo) | execution evidence derivable 🟢 | first-class immutable `harness-execution/v1` evidence; derived trust; Business Verification from recorded Finding bytes (D-I-5) | 🟡 | Themis commit; EDR-HARNESS-01 written |
...
| 13 | Cross-repo dependency walls | depguard allow-list on the harness adapter + `tests/architecture` exactly-one-importer 🟢 (I-M3, uncommitted) | wall 1 = no import of `themis-project/themis`, capability wall 🟢 (I-M4) | exactly one Themis package imports the harness; harness imports no Themis; no writer/exec across (D-I-7) | 🟡 | Themis commit |
