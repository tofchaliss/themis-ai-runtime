...
func TestParseAnchorRefusals(t *testing.T) {
	cases := []struct {
		...
		{"themis contract neither hash nor absent", func(m map[string]any) { m["themis_contract"] = "latest" }},
		{"themis contract missing", func(m map[string]any) { delete(m, "themis_contract") }},
