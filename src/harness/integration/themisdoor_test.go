...
// Themis read door (D-I-3), moved from the dissolved src/themis stand-in
// (I-M4). Positive path FIRST: under an anchor that pins the
...
func tdRepoRoot(t *testing.T) string {
	t.Helper()
	_, file, _, _ := runtime.Caller(0)
	return filepath.Join(filepath.Dir(file), "..", "..", "..")
}

func tdMirror(t *testing.T) (string, string) {
	t.Helper()
	git := gitBin(t)
	root := t.TempDir()
